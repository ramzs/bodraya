  ...
  if (fields.length) {
    Array.prototype.forEach.call(fields, function (field) {
      ...
      field.addEventListener('click', function (event) {
        if (event.target.classList.contains('field-num__button') && !input.getAttribute('disabled')) {
          let num = parseInt(input.value);
          if (isNaN(num)) num = 0;
          if (event.target.classList.contains('field-num__button--plus')) {
            if (num < valueMax) input.value = num + valueStep;
          }
          if (event.target.classList.contains('field-num__button--minus')) {
